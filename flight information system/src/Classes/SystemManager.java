...
//                      end = true;
    ...
    public String getDeparturedate(String flightID){
     ...
     public Reservation makeReservation(int number, Docente[] passengers,String flightID,String deptDate,Seat[] seat)
     {         
         reservationNumber++;
         
         Reservation reserve = new Reservation(reservationNumber,number, docenti, flightID, deptDate,seat);
    ...
    //load reservation details from file to array
     ...
     public void loadReservations(){
         try(Scanner read = new Scanner(new FileInputStream("reservations.txt"))){             
            ...
            Docente[] passengers = new Docente[numberOfPassengers];
            String flightID = "";
                    ...
                    while(!read.hasNext("##")){                        
                        for(int q=0; q<=reservationNumber; q++){
                            ...
                            for(int d=0; d<passengerDetails.length; d++){
                                ...
                                Nome nome = new Nome(singlePassengerDetails[0], singlePassengerDetails[1]);
                                Cognome cognome = new Cognome(singlePassengerDetails[0], singlePassengerDetails[1]);
                                Insegnamento insegnamento = new Insegnamento();
                                CodiceDocente codice = new CodiceDocente(singlePassengerDetails[2], singlePassengerDetails[3]);
                                docenti[d] = new Docente(nome, cognome, insegnamento, codice, singlePassengerDetails[4]);
                            }
                            ...
                            Reservation reserve = new Reservation(reservationNumber, numberOfPassengers, docenti, flightID, departureDate,reservedSeats);
                        }
                    }
                    end = true; 
                    line = "";
...
//
        ...
        public Docente[]  viewPassengerList(String flightID){
            String ftID = "";            
            Docente[] docenti = new Docente[448];
            int passengerCount = 0;
            for(int i=0; i<=reservationNumber; i++){
                ftID = reservations[i].getFlightNumber();
                if(ftID.compareTo(flightID)==0){
                    docenti = reservations[i].getDocentiArray();
                }
            }
            return docenti;
